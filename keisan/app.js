...
function gKukuYomi(lv){
  if(lv==null) lv=ri(1,10);
  /* 九九暗唱表 (各段9個ずつ。読み(全文)で記憶) は shared/kuku_phrases.js が単一の
     出どころ。木漏れ日の小道の段暗唱・れんぞく九九と同じ読みを使うため、ここに
     表を持たない。 */
  var PH=window.Q4B_KUKU_PHRASES;
  if(!PH)throw new Error("九九の読みデータが読み込まれていません");
  var KUKU=PH.table;
  /* 段の値(N)とdan順位の対応 */
  var dans=PH.teachingOrder;
  var dan;
...
var FURI5_PAIRS=[
  ...
  ["見学","けんがく"],["比べ","くらべ"],
  /* 木漏れ日の小道の画面語。「木漏れ日」は 木/日 の単字より前に置く。 */
  ["木漏れ日","こもれび"],["小道","こみち"],["地域","ちいき"],["世界","せかい"],["地図","ちず"],["行き","いき"],
