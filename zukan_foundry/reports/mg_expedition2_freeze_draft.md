...
- 2026-08-21: 発案者が 80 種案 (案 A、写真既存のみ) を選択し、これで freeze。案 B の追加 fetch 分を待たずに巻を確定する (fetch round の取得結果は将来の巻・差し替え候補として温存)。A- 等級 5 種と B 等級 7 種は現行写真のまま収録。対象更新は本文冒頭の 更新 6 ではなく **更新 5 (release:5)** に確定 (オーストラリア遠征 II が release:6 で仕込み済みのため、2026-08-18 の入れ替えは適用せず更新カレンダー 2 章の原記載どおり)。カテゴリ帰属は 6 章の推奨案 (ボルネオ遠征 I 由来の 4 本)。同日、反映一式 (bugs.js 80 種確定 + zukan_catalog 80 entry + volume_fixture_madagascar_2 の発射待機仕込み) を commit した。
