...
作成日: 2026-08-18。対象: `zukan_foundry/reports/borneo_expedition1_freeze_draft.md`（66 種未命名）、`zukan_foundry/reports/costarica_expedition1_freeze_draft.md`（76 種未命名）。

## 0. 前提と方法

- 命名規約は `docs/komorebi_naming_convention.md` (v0.1) に従った。名前の形は「修飾語 (写真で確認できる色・模様・形、または地域名) + 分類群の和名」、全体 4〜12 文字、学名のカタカナ転写禁止、写真で確認できない生態語の使用禁止、既存和名との衝突禁止。
- 対象は両 draft の選抜種のうち標準和名 (nameStatus: standard) を持たない種。ボルネオ 66 種 (SSR1/SR5/R11/N49)、コスタリカ 76 種 (SSR1/SR7/R13/N55)、計 142 種。英名候補 (nameStatus: english_common_candidate) であっても和名そのものは未確定のため命名対象に含めた。
...
- 衝突チェックは `shared/bugs.js` から正規表現で抽出した既存和名 1,765 件 (jaName 全件、コアデータ + JSON 形式の areaOnly komorebi 種を含む) と、本レポート内の両地域 142 件の提案同士を突き合わせて行った。詳細は 3 章。
...
| catharsius_dayacus | *Catharsius dayacus* | ダヤクコガネムシ（仮称） | 種小名 dayacus はボルネオ島先住民「ダヤク」に由来する地域名から修飾語とした。(写真未確認、学名/分類からの暫定) | なし |
...
| species_id | 学名 | 提案和名（仮称） | 由来 | 写真の有無 |
|---|---|---|---|---|
| atta_cephalotes | *Atta cephalotes* | トゲアカアリ（仮称） | 赤褐色の体色と胸部のとげ状突起が写真で確認できることから。看板種で通称「ハキリアリ」だが、葉を切り運ぶ行動は生態語のため規約上使えず、写真で確認できる外見的特徴から命名した。4 章に補足あり。 | あり |
...
- 既存和名: `shared/bugs.js` から `jaName` を正規表現で全件抽出し 1,765 件の一意な和名一覧を作成 (コア種の JS リテラル形式 111 件 + areaOnly komorebi 種を含む JSON 形式 1,654 件)。本レポートの提案 142 件を全件この一覧と突き合わせ、**完全一致の衝突は 0 件**だった。
- 相手 draft との突合: ボルネオとコスタリカを同一の 142 件プールとして突き合わせた結果、初回集計で以下 5 組・10 種が完全一致していた。いずれも別の写真根拠に基づき修飾語を変更して解消し、最終的に 142 件全てが一意になった。
...
- 文字数チェック: 142 件全てが 4〜12 文字の範囲内。
- 末尾チェック: 142 件全てが分類群の和名 (科の通称、無ければ目・大群の通称) で終わっている。
- カタカナ転写チェック: 目視で全件確認し、学名の音をそのまま転写した例は無い。地域名 (ボルネオ、カリマンタン、ダヤク、カリブ、バラバク) は規約が認める「地域名」修飾語として使用した。
...
2. **看板種 Atta cephalotes の名称方針**: コスタリカ看板は通称「ハキリアリ」で知られるが、葉を切って運ぶ採葉行動は標本写真からは確認できない生態語にあたるため、規約 (してはならないこと 2) により名称には使わず、写真で確認できる赤褐色・棘のある体形から「トゲアカアリ」とした。「ハキリアリ」系の名称を採用する場合は規約の例外扱いとする判断が必要になる。
3. **民族名を含む提案 (1 種)**: `catharsius_dayacus` の「ダヤク」はボルネオ島先住民族の呼称で、学名の種小名 dayacus に由来する地域文化名としてそのまま用いた。地域名としての使用可否は確認をお願いしたい。
4. **写真未確認 (学名/分類のみからの暫定) の種数**: ボルネオ 17 種、コスタリカ 19 種、計 36 種。これらは freeze 前に標本写真の確保・実見が必須。
