...
quest4bugs 本編に新規昆虫 150 種 (batch 1) を追加し、全自動テスト green の状態でリリース可能な commit を作る。学習コンテンツの変更はなし (species-only 拡充)。写真は後追いのため zukan_catalog.js は変更しない (全 150 種が parametric SVG fallback で描画される)。

## 入力 (すべて存在済みが前提)

- entry データ: `zukan_foundry/data/species_reserve/honpen_batch1_entries_{keisan,kanji,eitango}.jsonl` (各 50 行、1 行 1 JSON、全キー引用符付き)
...
新種 150 (keisan 50 / kanji 50 / eitango 50、order 由来で自動割当) により:

- `tests/test_zukan_progress_count.js`: 総種数 1213 → 1363、分母 427/353/423 → 477/403/473、「分母合計 + 天敵 10 = 総種数」の検証は 1363 に
- `tests/test_species_migration.js` ほか、1213 やプール数 (380/333/408 → 430/383/458) を固定している assertion を全 grep して更新
- `zukan_foundry/tests/test_foundry.py`: `len(parsed) == 1213` → 1363。media 数 892 は不変
...
- Conventional Commits で 1 commit (例: `feat(zukan): add 150 species (expansion batch 1)`)。push は行わず、commit 作成まで (push は人間が確認後)

## 受け入れ基準

1. validator exit 0、レビューリスト生成済み
2. 全 node テスト + foundry テスト green
3. bugs.js の種数が正確に 1363、新 id 重複ゼロ
