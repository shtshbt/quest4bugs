(function(global){
  ...
  /* 2026-09-01 に 0.85 から 0.70 へ下げた。合格線は Lv の昇降帯 (app.js applyPerformance:
     10 問窓で 8 問以上なら昇格、4 問以下なら降格) と揃えて読む値であるべきで、0.85 は
     「Lv10 に居座れている」(維持帯) より高い位置にあった。そのため Lv10 を保てている
     のにメダルだけ永久に出ない帯が広く開き、条件表示が動かないまま残った。
     0.70 = 20 問中 14 問は、降格帯 (10 問中 4 問以下) を 2 ブロック続けたら届かない
     一方、昇格帯の下限 (8/10) を 2 ブロック保てば越える。「Lv10 を維持できる実力を
     2 周分」という設計意図をそのまま数にしたもの。 */
  var STABILITY={windowSize:20,minAnswers:20,minAccuracy:0.70};

  /* リセット周回 (tools_design 5 章)。Lv10 クリアから 7 日の暦ゲートを置き、その
     あとで本人が選んだときだけ Lv1 に戻す。週 1 回程度の全周回 (Lv1-10 + 安定判定
     = 実質 100 問超の 70% 正答) は farming ではなく復習で、暦時間のロックが上限を
