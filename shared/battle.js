/* Quest4Bugs バトルエンジン（共有）。ボスロスター＋相性・ダメージ・HP・解禁ロジック。
   ...
   有利+1ダメージ/不利-1。技は一律。レア度=HP(粘り)。ボスは習得ゲージで段階解禁。 */
(function(global){
  "use strict";
  ...
  var BASE_DMG = 2, ADV = 1, DIS = -1, BOSS_DMG = 1;
  /* attacker属性 が boss属性 に対して与えるダメージ */
  function damage(attackerType, bossType){
    if(!attackerType || !bossType || bossType==="none") return BASE_DMG;
    if(BEATS[attackerType]===bossType) return BASE_DMG + ADV;   // 有利
    if(BEATS[bossType]===attackerType) return Math.max(1, BASE_DMG + DIS); // 不利
    return BASE_DMG;                                            // 普通
  }
