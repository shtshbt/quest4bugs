...
16. **問題数/かけらコストは確定**: 問題数 N=100/R=250/SR=500/SSR=750/SS=1000、かけらコスト N=3/R=6/SR=9/SSR=12/SS=15 (低コスト・育成重視)
...
種に依存せず、レア度のみで決定 → 公平。
**低コスト・育成重視**: かけらは産卵入口の象徴的コストに留め、ゲームの主要な負荷は学習正解数 (育成) に集約。子供が長期目標を持ちやすい。

実装: `eggCost(sp) = [3,6,9,12,15][tierOf(sp)]` / `eggTarget(sp) = [100,250,500,750,1000][tierOf(sp)]`。
`spendForEgg(coll, sp)` が `fossilFragmentsStore.spend(eggCost(sp))` を内部で呼ぶ。
進捗表示は分母明示形式「+1 (47/100)」「+1 (823/1000)」に統一。
...
│ │   │コスト 🔶 15          │ │
│ │   │♂3 / ♀1 持ってる      │ │
│ │   │※ かけらが たりない    │ │ ← disabled (例: 残かけら 8 のとき)
│ └────┴─────────────────────┘ │
│ ┌────┬─────────────────────┐ │
│ │SVG │オオクワガタ           │ │
│ │ 🟣 │ウルトラレア / 300問   │ │
...
│ ────────────────────────── │
│ 🥚 そだち中:                 │
│  🛌 オオクワガタ +1 (742/750)│  ← 孵化準備に近い (先に表示、SSR=750)
│  🐛 ヘラクレス  +1 (253/1000)│  ← progress 順 (SS=1000)
...
[最近5件の表] ← 🐣 マーク付き行は太字背景色
🥚 たまごを 産ませる (🔶 15) ← 前提クリア時のみ (disabled+理由は前提未充足時、ヘラクレス=SS のコスト)
...
- [x] ~~かけらコスト数値の最終決定~~ → 問題数 N=100/R=250/SR=500/SSR=750/SS=1000、コスト N=3/R=6/SR=9/SSR=12/SS=15 で確定 (低コスト・育成重視。1日30問でかけら +1 ペースゆえ高コストは到達不能)
...
- 卵からかえした `reared:true` の個体を持つ種は、SS を含めバトルで常時 HP +2。
- かせきのたにでは HP パーツの floor を先に適用し、その後で +2 を加算する。パーツの説明「HPを20にする」は変更せず、そだてた効果を独立した上位レイヤーとして扱う。
- 平等原則は「育てる + パーツ」で N / R / SR / SSR の全種が HP 22 に並ぶことで維持する。育てた SS は HP 22。
- 常時 HP ボーナスは +2 に固定する。将来の育成強化は HP ではなく、きずな等の別軸で検討する。
