...
| 4 | 雫 | **小道非対応**。雫は本編の資源なので、色違い化 picker の対象・使途とも本編だけにする |
| 5 | 朝露ボーナス (6-8 時に色違い出現率上昇) | **小道の捕獲抽選にも適用する**。朝に触るインセンティブは小道でも同じに働く。抽選器が本編と共通ならフラグ確認のみで済む可能性があるが、コード確認が先 (下記 2-4) |
| 6 | breeding UI | **小道の図鑑詳細にも本編と同じ育成セクションを出す**。スロット一覧・待機列・受け取りの管理は御神木パネルに一元化する |
| 7 | 色違い集計 | **サマリーと称号に小道の色違い種も含める**。雫の色違い化 picker は本編だけのまま |
| 8 | records | **件数は無制限のまま様子を見る**。save.json が 500KB を超えたときだけ、1 セッション 1 回 console.warn で監視する |
| 9 | 小道スロット段の「＋」 | **全体ピッカーのまま**。小道種だけに絞らない |
...
1. ~~breeding UI での小道種の露出範囲~~ → 2026-08-14 決定: 小道の図鑑詳細にも本編と同じ育成セクションを出す。卵の管理は御神木パネルに一元化し、小道ページに別の管理パネルは作らない。小道種は御神木の卵ピッカーにも並び、産まれた卵は御神木パネルの小道専用スロット段に表示される
2. 朝露ボーナスの正本仕様の文書化先 (本書に追記するか独立文書にするか)
3. ~~雫の最終判断~~ → 2026-08-14 決定: 雫は本編の資源なので、小道では増えず、色違い化 picker も本編だけを対象にする
