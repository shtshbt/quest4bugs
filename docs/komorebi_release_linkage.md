...
第 4 列 (k10) と第 5 列 (k5) が機械可読の正本である (`tests/test_komorebi_release_gate.js`
が読み、実装の `CATEGORIES` と突き合わせる)。cat id を `+` で連ねる。未定の枠は「なし」。
第 6 列の道具は `shared/tools.js` の各道具の `release` の写しで、正本はそちら。

| 更新 | 地域 volume | 種数 | k10 (2 本以上) | k5 (2 本以上) | 採集道具 | 写真 | 巻の状態 |
|---|---|---:|---|---|---|---:|---|
...
k10 2 + k5 2 の新しい規則に従う。道具は更新番号だけでは開かず、`shared/economy_flag.js`
の `MEDAL_ECONOMY_ON` が前段のスイッチになっている (2026-08-21 に点火済み)。

在庫の残り。

| 種別 | 全数 | 更新 5 までに投入 | 更新 6 以降に使える残り |
|---|---:|---:|---:|
| k10 学習カテゴリ | 10 | 10 | 0 |
| k5 学習カテゴリ | 13 | 13 | 0 |
| 採集道具 | 15 | 11 | 4 (更新 6 に 2 本、更新 7 に 2 本) |

学習カテゴリは更新 5 で k10 も k5 も使い切る。

更新 6 の出し方は 2026-09-07 に決めた。新しい学習カテゴリを作ってから巻を出す。
巻だけ先に出す案 (2.0 の規則を「巻を伴う更新に限る」と読み替える) と、道具だけの更新に
する案も並べたが、採らない。規則が守っているのは「5 歳コースの子が学習を経ずに図鑑を
埋められない」という一点で、こはく呼び出しがコースを見ない (`amberCallVolume`) 以上、
巻を出すなら k5 のカテゴリが要る。規則を緩めるとその一点が失われる。

したがって更新 6 の律速は巻の準備ではなく新カテゴリの開発になる。必要なのは
k10 2 本と k5 2 本。roster (`komorebi_categories.md` 2 章) にある未実装の候補は
kom_exhaustive_search (k5、Lv 表は 3.7 章にあり、新 kind「集合完成」の実装が前提) の
1 本だけなので、残る 3 本 (k10 2 + k5 1) は起案から始める。
...
### 2.2.1 更新 6 の巻には朽ち木性の甲虫が要る (2026-09-06 実測)

更新 6 で開ける道具のひとつが材割りセット (`komorebi_tools_design.md` 6.3) で、
対象は幼虫が材を食う家系 (クワガタ・カミキリ・タマムシ・コメツキモドキ・ゴミムシダマシ)。
この道具が働くには、巻の 84 種のうち 5% 以上 = 5 種前後がその家系である必要がある。

地域 seeds を数えたところ、既存 4 地域はどれもこれを満たせない。

| 地域 seeds | 種数 | 甲虫 | 朽ち木性 |
|---|---:|---:|---:|
| マダガスカル | 301 | 33 (11.0%) | 2 (0.7%) |
| オーストラリア | 302 | 31 (10.3%) | 3 (1.0%) |
| ボルネオ | 200 | 22 (11.0%) | 0 |
| コスタリカ | 301 | 30 (10.0%) | 0 |
| フィリピン | 600 | 70 (11.7%) | 23 (3.8%) |
| ニューギニア | 600 | 78 (13.0%) | 18 (3.0%) |
| 南部アフリカ | 600 | 59 (9.8%) | 3 (0.5%) |

甲虫そのものは どの地域も 10% 前後あるので、足りないのは甲虫ではなく朽ち木性の家系
だけである。既存 4 地域から 84 種を引くと、朽ち木性は 0 か 1 種にしかならない。

したがって選べる道は 2 つ。

1. 更新 6 の巻を Tier 2 (フィリピンかニューギニア) から出し、freeze の種選抜で
   朽ち木性を 5 種以上 意図的に入れる。比率どおりに引くと 3 種前後 (3.6%) で下限を
   割るので、選抜の時点で指名する必要がある
2. 材割りセットを本編 (けいさん、30.6%) 専用として出す。小道では「この遠征では
   出番がない」と出たままになるが、道具そのものは働く

どちらを採るかは更新 6 の巻を決めるときに判断する。1 を採るなら、Tier 2 地域の
harvest と freeze が更新 6 の律速になる。
