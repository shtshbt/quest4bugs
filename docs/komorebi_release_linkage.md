...
準備の進み具合であって、順番の予約ではない。整った巻から空いている最小の枠へ入れる。
manifest の `release` には未割り当ての印として 99 を置く。`isVolumeReleased` は release が
整数でない巻を公開済みへ倒すため、null や undefined は使えない。

| 巻 | 在庫 | freeze draft | 命名 | GBIF 探索 | 写真 | 残る作業 |
|---|---:|---|---|---|---|---|
| コスタリカ遠征 I | 294 | あり (84 種選抜案) | 75 種の仮称提案済 | 78/84 | 0/84 | 選抜の承認、画像取得と切り抜き、bugs.js 登録、manifest 実データ化 |
| ボルネオ遠征 II | 約 116 | なし | なし | なし | なし | 種選抜から全工程 |
| コスタリカ遠征 II | 210 | なし | なし | なし | なし | 種選抜から全工程 |

コスタリカ遠征 I が最も進んでいる。写真は 0 枚だが、律速である GBIF の探索 (レート制限
2 秒/リクエストで種ごとに最大 8 階層を叩く工程) が 78 種ぶん終わって `_pipeline` に
残っており、命名も仮称提案まで済んでいる。残るのは画像取得と切り抜きという後半工程で、
ここは GBIF のレート制限を受けない。

写真の枚数だけで順番を決めない。ボルネオ遠征 II とコスタリカ遠征 II は draft すら無く、
種選抜から始める必要がある。
...
- guild カバレッジ確認: 公開中の各道具に対象種が下限 (5%) 以上いるか。対象ゼロの道具が残ったまま公開すると、道具だけ出て実際には何も拾えない。2026-09-06 以降は手作業ではなく `node tests/test_species_guilds.js` が見る。新しい巻を `komorebi/volumes/` に足したら、この test の被覆表 (`EXPECTED`) に列を 1 本足して実測値を書き込む。数字が動いたら test が止まるので、道具が死札になったまま公開されることはない。
