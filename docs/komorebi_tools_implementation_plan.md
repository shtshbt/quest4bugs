...
4. 済 道具図鑑 (初回授与の記録)
5. 済 (2026-09-06、別解) 種データへの遡及追記はしない。判定を採集ギルド層へ移し、主キーを分類にした (Phase 4 を参照)
6. 済 (2026-09-06) volume freeze チェックリストへの guild カバレッジ確認の追記 (`docs/komorebi_release_linkage.md` 4 章)
...
1. (決定済 2026-08-21: 対応しない) リセット周回はゲージの供給を落とさない。
   発案者の判断で、2 周目以降もゲージ減衰は入れない (8 問 1 匹の約束を優先し、
   周回の実時間コスト低下は許容する)。発行速度の実測監視は継続する。
   — 元の観測: こはくは `maxLv` で 0.4 に減衰する
...
### Phase 4: 採集ギルド層 (2026-09-06)

未公開 5 種 (ビーティングセット / 吸虫管 / 高所用長竿 / 落とし穴トラップ / フントラップ) を
公開できる状態にし、今後トラップを足すときの入口を 1 か所へ寄せる。branch は
`claude/komorebi-tools`。設計判断は `komorebi_tools_design.md` 6.1 と 6.2 が正本。

やったこと。

| 対象 | ファイル | 内容 |
|---|---|---|
| ギルド層 | `shared/species_guilds.js` (新規) | ギルド 11 種。判定は order / family / 属名を主キーに、habitat と tags を補助キーに置く |
| 道具 | `shared/tools.js` | 各道具の `match` 関数を `guildKey` 1 本へ置換。分類の集合はすべてギルド層へ移した。`tool.guild` (対象 guild の説明) はギルド層から引く accessor になり、道具側に文言の複製が無くなった |
| 配線 | 6 ページ + `sw.js` | `shared/species_guilds.js` を `shared/tools.js` の直前に足す。`?v=0.1.0`、`shared/tools.js` は 0.2.4 から 0.2.5 へ、CACHE は v171 から v172 へ |
| 監査 | `tests/test_species_guilds.js` (新規) | 5 巻 + 本編 3 教科 × 11 道具の被覆表、道具とギルドの 1 対 1、主キーの充足、飲み込みの上限 80%、止水の境界 |

実測の効果 (移行前 → 移行後)。

- 高所用長竿: 全 DB 1 種 → 113 種。5 巻すべてで下限超え
- フントラップ: 8 種 → 53 種。公開回の巻 (マダガスカル遠征 II) で 5.0%、けいさんで 7.7%
- 落とし穴トラップ: 95 種 → 202 種。オーストラリア遠征 I 以外の 4 巻で下限超え
- 灯火 / スイーピング / 吸虫管: 下限割れだった巻 (AU I 4.8% / BO I 1.2% / BO I 3.6% / MG II 0%) がすべて解消
- どの道具の対象でもない種: 45 種 → 4 種 (昆虫以外の天敵枠のみ)

公開済み 6 種の被覆は 1 プールも下限を割っていない (バナナトラップの AU II が 11.9% から
7.1% へ、さかなとりあみ の MG I が 41.7% から 25.0% へ下がったが、どちらも下限の上)。

残っていること。

- 更新カレンダー 2.1 (2026-08-28 再編) に道具列が無い。`shared/tools.js` の `release` は
  旧カレンダー基準のままで、いまは 更新 4 = オーストラリア遠征 II、更新 5 = マダガスカル
  遠征 II と読み替える必要がある。読み替えたうえで被覆は足りている (AU II のビーティング
  33.3% / 吸虫管 22.6%、MG II の長竿 5.0% / 落とし穴 28.8% / フン 5.0%)
- ちょうネットの AU II (2.4%) と 落とし穴トラップの AU I (2.4%) は下限割れのまま。どちらも
  その巻の種構成がそうなっているだけで、`worksInAny` が他の巻で救う。無理に広げない
- フントラップは 11 種で最も専門的で、下限を超えるのは MG II とけいさんだけ。`GUILD_MIN_SHARE`
  を 5% から 4% へ下げれば BO I (4.8%) と AU II (4.8%) も入るが、下限は全道具に効く定数なので
  ここでは動かさない。動かすなら別の決定として

### Phase 5: 第 2 波の 4 種 (2026-09-06)

マレーゼトラップ / 材割りセット / サーバーネット / 衝突板トラップ を、更新 6 と 7 で
開ける状態にした。設計判断は `komorebi_tools_design.md` 6.3 から 6.5 が正本。

| 対象 | ファイル | 内容 |
|---|---|---|
| ギルド | `shared/species_guilds.js` | `malaise` / `deadwood` / `stream` / `flight_beetle` を追加。あわせて `grassland` から ハチ目とハエ目、`hidden` から朽ち木性の家系、`aquatic` から渓流のみの種を外した |
| 道具 | `shared/tools.js` | `malaise_trap` (更新 6) / `deadwood_set` (更新 6) / `surber_net` (更新 7) / `window_trap` (更新 7) |
| 絵 | `shared/tool_icons.js`, `shared/tool_scenes.js` | アイコン 4 枚と捕獲ビネット 4 枚 + 1 行の文 |
| 配線 | 6 ページ + `sw.js` | 版を上げただけ (新規ファイルなし)。CACHE は v172 から v173 |
| 監査 | `tests/test_species_guilds.js` ほか | 被覆表を 15 行へ、道具の数を 11 から 15 へ |

被覆 (2026-09-06 実測、当たり数)。

| 道具 | MG I | AU I | BO I | AU II | MG II | かんじ | けいさん | えいたんご |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| マレーゼトラップ | 2 | 7 | 8 | 4 | 4 | 0 | 0 | 113 |
| 材割りセット | 3 | 1 | 0 | 2 | 1 | 0 | 147 | 0 |
| サーバーネット | 10 | 3 | 9 | 4 | 6 | 0 | 8 | 42 |
| 衝突板トラップ | 3 | 8 | 4 | 10 | 4 | 0 | 213 | 0 |

削った側の変化。スイーピングネットは 497 から 365 種 (えいたんご 50.6% から 29.9%)、
ビーティングセットは 457 から 350 種、さかなとりあみは 245 から 204 種。公開済みの
6 種はどのプールでも下限 (5%) を割っていない。

飲み込みの最大は フントラップと吸虫管の 70%、材割りとバナナの 70% で、上限 80% の内側。

残っていること。

- 更新 6 の巻に朽ち木性の種 (カミキリ・クワガタ) を入れる。2026-09-06 に地域 seeds を
  数えたところ、既存 4 地域 (マダガスカル 2 / オーストラリア 3 / ボルネオ 0 / コスタリカ 0)
  はどれも足りず、84 種を引くと 0 か 1 種にしかならない。持っているのは Tier 2 の
  フィリピン (23) とニューギニア (18) だけで、それも比率どおりに引くと 3 種前後 (3.6%) で
  下限を割る。選択肢と判断の材料は `komorebi_release_linkage.md` 2.2.1 にまとめた
- 更新カレンダー 2.1 に道具列が無いのは Phase 4 から据え置き。`shared/tools.js` の
  `release` は 6 と 7 まで伸びたが、更新 6 以降の巻はまだ枠のみ

### Phase 6 (未着手): 卵と幼虫の採集

たまごさがし / 幼虫さがし。ギルドを持たない道具として、捕獲の形を変える。
方向は `komorebi_tools_design.md` 6.5 に置いた。着手は第 2 波の公開後。
