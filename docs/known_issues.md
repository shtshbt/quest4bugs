...
依存関係と被害規模で並べた実施順序。 PB-3 (presence) は CAS 移行完了後にする
(presence は事前警告であり、 競合の実防止は CAS が担うため):

```
0. T4 を resolved に整理 (本 commit)
1. T8 個人情報の新規流入を止血 (cloud は pseudonym + local 対応表)
2. T7 eggUid 導入 (breeding namespace 内 schema 変更、 PB-2 breeding の続き)
3. T10 装備の単一真実源化 (equippedBy を導出専用に)
4. PB-2 残 namespace を CAS 移行:
   wallet → battle → keisan → kanji → eitango
   (wallet を先頭に置く理由: 経済破綻 = 子供体験の直接破壊)
5. 旧 save() を CAS namespace で throw 化 (将来の迂回禁止)
6. T6 version 整合 boot (まず APP_RELEASE_ID マッチ、 後で manifest)
7. PB-3 presence (BroadcastChannel + cloud heartbeat + 引継ぎ UI)
8. K16 field-level merge (実質 Phase C2、 別プロジェクト扱い)

並行: K9 (時刻 別入力), K13 (kanji_data validator)
後回し: K14, K17, K18
```

### 直近報告で未対応 (T 系列)

- ~~**T4 クラウド push queue の偽 synced**~~ → v120 (PA-1) で dirty generation 方式に修正済
- **T6 デプロイ更新中の混成 boot** — HTML が新、 JS が旧 caches 返却で migration が旧ロジックで実行される。 まず APP_RELEASE_ID 整合チェック (小)、 後で content hash + manifest (大)。
- **T7 eggUid 導入** — species ID を卵 ID 兼用しており、 同種の異なる卵 (♂/♀, 通常/色違い, 進捗差) を区別できない。 crypto.randomUUID ベース uid に。 PB-2 残 namespace 移行より先。
- **T8 GitHub commit に名前・履歴が残り続ける** — snapshot 高頻度 commit が past commit に残存。 まず新規流入を止血 (cloud は pseudonym 化 + ローカル `_meta` に対応表)、 後で history rewrite or repo rotate。
- **T10 装備の二重インデックス** — equipped + equippedBy が片方欠落時に補完されず、 唯一品の複製。 equippedBy を導出専用 (保存しない) に。 PB-2 battle 移行直前に実施。
...
### v122 (今回追加) — Phase B-2 (breeding 完全移行)
- shared/reward.js: breeding adapter を {loadVersioned, saveVersioned, pid} 対応に
  拡張。 内部に __bsCache を持ち _ensureBsLoaded/_saveBs を async 化
- breeding 操作 API を全て Promise 化 (layEgg/awardMasterEgg/awardBossEgg/feedEgg/
  hatchEgg/acceptPendingEgg/promotePendingEgg/demoteEggToPending/discardPendingEgg/
  abandonEgg/setMasterSex/migrateUnstartedLegacyEggsToPending/autoFinalizeLegacyMasterAll/
  advanceStage)
- caller 全箇所を await 化: kanji/keisan/eitango (layEgg/abandonEgg/hatchEgg/
  setMasterSex/awardMasterEgg)、 root index (autoHatch 順次/手動 hatch/layEgg/
  abandonEgg/promote/demote/discard/advance/accept/setMasterSex)、 battle.html
  (awardBossEgg) と shared/boss_zukan.js (legacy 救済)
- hatchEgg は 2 phase: breeding 保存成功後に coll.records を追加 → caller 改修済
- shared/breeding.js に q4b-breeding-conflict event 購読モーダル (即時 reload せず
  「さいしんの データを よみこむ」 ボタン確認制) を追加
- mergeStore に same_revision_divergence 検出を追加 (warn + event)
- smoke test PASS:
  - 同 revision race → A 成功・B 競合 + backup 自動作成
  - legacy data (rev なし) → load で rev=1
  - listConflictBackups() で UI 参照可

**残:** PB-2 完了後の動作確認で旧 save() を CAS namespace で throw に格上げ。
PB-3 BroadcastChannel + presence。 他 5 namespace (keisan/kanji/eitango/battle/wallet)
の caller 移行。

### v121 (前回) — Phase B-1 (CAS インフラ)
- `QuestSave.loadVersioned(ns, key, default)` → `{data, revision, updatedAt, updatedBy, fresh}`
- `QuestSave.saveVersioned(ns, key, data, expectedRevision, opts)` → 成功時
  `{ok:true, revision:N}` / 競合時 `{ok:false, reason:"conflict", expectedRevision,
  actualRevision, remoteData, backupKey}`
- KV エントリに revision (logical) + updatedBy (deviceId) を持つように。 旧 save()
  でも +1 進めるので段階移行しても CAS が動作する
- CAS_NAMESPACES = kanji / keisan / eitango / breeding / battle / wallet
- mergeStore を CAS aware に: remote.revision > local.revision なら必ず remote
  優先、 同 revision は時刻 LWW
- 競合 backup を q4b_conflict_backup_* に保存 (profile 10/全体 30/30 日 TTL)、
  localData / remoteData / expectedRevision / actualRevision / deviceId 付き
- listConflictBackups(profileId) / readConflictBackup(key) で UI から参照可能
- device id を q4b_device_id_v1 に永続化
- caller 移行は PB-2 で namespace ごとに段階実施 (旧 save() は warn log 出力中)

**残:** PB-2 caller を loadVersioned/saveVersioned に段階移行 (breeding → battle →
keisan → kanji → eitango → wallet)。 PB-3 BroadcastChannel + presence + 引継ぎ。

### v120 (前回) — Phase A (同期基盤の改善)
- **PA-1 + T4 同期 status 拡張 + push queue dirty generation**:
  status を local/dirty/syncing/synced/auth-error/offline/conflict/error に拡張、
  偽 synced (設定だけで synced 表示) を廃止。 persist() で localGeneration を進めて
  status="dirty" に。 pushSnapshot を dirty generation 方式に変更、 後追い push も
  pushAll を経由して status / 最終成功時刻を更新するように。
- **PA-2 PAT 監視 + エラー分類**:
  GitHub PUT で 401/403 を即時失敗扱い、 _classifyError で auth-error/offline/
  conflict/error を区別。 lastSuccessAt / lastErrorAt / lastErrorKind を
  LAST_SYNC_KEY に永続化。 GitHub-Authentication-Token-Expiration header を保存して
  PAT 期限を表示できるように。 QuestSave.getSyncMeta() で UI から取得可。
- **PA-3 永続ストレージ要求**:
  QuestSave.requestPersistent() / storageEstimate() を追加 (Storage API ラッパ)。
  root 親画面に「📌 端末ストレージを 永続にする」 ボタン + 「☁️ 同期じょうたい
  をみる」 で最終成功時刻・PAT 期限・使用量・永続状態を表示。

### v119 (前回)
- **U1 battle.html の残存ボス卵救済**: 同実装に残っていた n>=1 救済を n>=10 に
  統一 + _battleBackfillDirty で「変更時必 ず save」 を保証。 T1 の修正が別
  入口で生き残っていた致命的経路を完全閉鎖
- **U2 autoHatch 内の pid 不一致**: load 完了後と各 hatchEgg 呼出前に
  currentProfile() == pid を再確認、 別の子の卵ストア操作を遮断
- **U3 卵 validator 厳格化**: game/sex 必須化、 Q4BReward.spById 実在 +
  eggGameFor 一致を要求。 breedingOf で隔離発生時に canonical store へ書戻し
  (旧 clone-only normalize の片手落ちを解消)
- **U4 強制復元の完全置換**: kv/profiles/current/tombstones を obj で置換、
  updated を全件打ち直し、 obj.current も復元。 旧「overlay」 から本来の
  「過去時点へ戻す」 動作に
- **U5 ボス二重計上**: ポータル集計で教科 coll + bosses の集合 union で
  bossOnlyN を加算 (= 教科に登録済の boss は重複排除)、 pct を Math.min(100,...)
  でクランプ
