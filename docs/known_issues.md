...
### keisan の中優先 (今回追加報告の 8-11)

- **K19. 名前変更が計算側だけに反映**。 `renameP()` で `QuestSave.updateProfile(id, {name})`
  経由に統一し、 共有レジストリも同期。
- **K20. ミッションから基本カテゴリ 2 つ (`kukuyomi`, `warizan`) が永久に除外**。
  九九は二重に入っているため、 例えば未学習・低 Lv・久しく触っていないカテゴリ
  を優先する選択ロジックに切替。
- **K21. 「にがした虫」履歴の FIFO 重複** — 同概念で複数回間違えると別エントリで
  40 件枠を埋め、 古い未解決問題が押し出される。 `cat + level + patternId` で
  重複統合 + tries/due 更新。
- **K22. 平均算で 100 点超のテスト得点** が出る。 テスト文脈なら 0〜100 に制限。
